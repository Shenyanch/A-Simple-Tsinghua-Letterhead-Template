    ...
    \begin{itemize}
      \item 几个特殊的集合：
        \begin{itemize}
          ...
          \item \mysout{全体{\fangsong 复数}组成的集合，记作$\mathbb{C}$(complex)}.
        \end{itemize}
      ...
      \item 如果集合$A$中的元素都属于集合$B$，那么称
        \begin{itemize}
          ...
          \item 集合$B${\fangsong 包含}(contain)集合$A$，记作$B\supseteq A$ \mysout{，并称集合$B$是集合$A$的{\fangsong 超集}(superset)}；
        \end{itemize}
        进一步，如果$A\subseteq B$，且$A\neq B$（即：$B$中含有集合$A$中没有的元素），则称
          \begin{itemize}
            \item 集合$A$是集合$B$的{\fangsong 真子集}(proper subset)，记作$A\subsetneqq B$；
            \item \mysout{集合$B$是集合$A$的{\fangsong 真超集}(proper superset)，}记作$B\supsetneqq A$；
          \end{itemize}
        {\kaishu
...
\section{复数}
  \subsection{定义}
    \begin{itemize}
      ...
      \item 当且仅当两个复数的实部和虚部分别相等时，两个复数{\fangsong 相等}.即：如果$a,b,c,d\inR$，那么
            ...
            {\kaishu \circled{注} 结论“若$x+y\ii=a+b\ii$，那么$a=c\text{且}b=d$”是错误的，因为$x$，$y$，$a$，$b$可能不是实数.}
  ...
  \subsection{复数的运算}
    ...
    \end{itemize}
    \eg{$(2+3\ii)-(-1+\ii)=3+2\ii$；$(2+3\ii)(-1+\ii)=-5-5\ii$；$\mfrac{1+\ii}{1-\ii}=\mfrac{(1+\ii)^2}{(1-\ii)(1+\ii)}=\mfrac{2\ii}2=\ii$；$(3+4\ii) \bar{3+4\ii}=3^2+4^2=25$.}\\
  ...
  \subsection{乘方与开方}
    \begin{description}[leftmargin=0pt]
      ...
      \item [$n$次方根与根式]
        ...
        %这里$n$被称为{\fangsong 根指数(radical exponent)}.
...
\section{三角函数概念、同角关系与恒等变换}
  ...
  \subsection{任意角的概念}
    \begin{enumerate}[1)]
      \item {\fangsong 角}可以看成平面内一条射线绕着端点从一个位置旋转到另一个位置所形成的图形.
        规定按逆时针方向旋转形成的角叫做{\fangsong 正角}，按顺时针方向旋转形成的角叫做{\fangsong 负角}.
        如果一条射线没有作任何旋转，则称其为{\fangsong 零角}.\\
        \eg{\begin{minipage}[t]{0.65\linewidth}\vspace{-0.5\baselineskip}
            如图，一条射线的端点是$O$，若从起始位置$OA$（{\fangsong 始边}）按逆时针方向旋转到终止位置$OB$（{\fangsong 终边}），形成$120\degree$的角；若从始边$OA$按顺时针方向旋转到终边$OB$，则形成$-240\degree$的角；若从始边$OA$按逆时针方向旋转一圈后继续旋转到终边$OB$，则形成$480\degree$的角；
          \end{minipage}
      ...
      \item 在直角坐标系内讨论角时，令角的顶点与原点重合，角的始边与$x$轴的非负半轴重合{\kaishu （始边是射线，应包括端点即原点，故不能说成正半轴）}.那么，角的终边在第几象限，就说这个角是第几{\fangsong 象限角}.
