...
\section{要点复习}
  ...
  \subsection{三角函数的图像和性质}
    \vspace{-1em}
    \begin{multicols}{2}%正弦函数与余弦函数
      \subsubsection{正弦函数}
        \begin{center}
          \begin{tikzpicture}[scale=0.8]
            \coordinate[label=below right:\small$O$] (O) at(-0.12,0.1);
            \coordinate[label=below :\small$-\piup$] (t1) at(-pi,0);
            \coordinate[label=below :\small$\piup$] (t2) at(pi,0);
            \draw[->,>=latex](-1.5*pi,0)--(1.5*pi,0)node[below](x) {$x$};
            \draw[->,>=latex](0,-1.8)--(0,1.8)node[right](y) {\small $y=\sin(x)$};
            \draw [domain=-1.2*pi:1.2*pi,samples=1000] plot(\x,{sin(\x r)});
            \draw[densely dashed](pi/2,0)node[below](pi){$\frac{\piup}{2}$}--++(0,1);
            \draw[densely dashed](-pi/2,0)node[above](-pi){$-\frac{\piup}{2}$}--++(0,-1);
        ...
        \begin{enumerate}[label=\circled{\arabic*}]
          \item 定义域：$x\inR$；\quad 值域：$ \left[-1,1\right] $ ；
          \item 奇偶性：奇函数；
          \item 最小正周期：$T=2\piup$;
          \item 对称轴：$x=k\piup+\dfrac{\piup}{2}\left(k\inZ\right)$；\\
                对称中心：$\left(k\piup,0\right)\left(k\inZ\right)$；
          \item 单调递增区间：$ \Bigl[2k\piup-\dfrac{\piup}{2},2k\piup+\dfrac{\piup}{2}\Bigr]\left(k\inZ\right) $；\\
                单调递减区间：$ \Bigl[2k\piup+\dfrac{\piup}{2},2k\piup+\dfrac{3\piup}{2}\Bigr] \left(k\inZ\right)$.
        \end{enumerate}
      \subsubsection{余弦函数}
        \begin{center}
          \begin{tikzpicture}[scale=0.8]
            ...
            \draw[->,>=latex](-1.5*pi,0)--(1.5*pi,0)node[below](x) {$x$};
            \draw[->,>=latex](0,-1.8)--(0,1.8)node[right](y) {\small $y=\cos(x)$};
            \draw [domain=-1.2*pi:1.2*pi,samples=1000] plot(\x,{cos(\x r)});
        ...
        \begin{enumerate}[label=\circled{\arabic*}]
          ...
          \item 奇偶性：偶函数；
          \item 最小正周期：$ T=2\piup $；
          \item 对称轴：$ x=k\piup \left(k\inZ\right) $；\\
                对称中心：$\left(k\piup+\dfrac{\piup}{2},0\right)\left(k\inZ\right)$；
          \item 单调递增区间：$ \left[2k\piup-\piup,2k\piup\right] \left(k\inZ\right)$；\\
    ...
    \subsubsection{正切函数}
      \begin{multicols}{2}
        \begin{enumerate}[label=\circled{\arabic*}]
          \item 定义域：$\Bigl\{x\Bigm|x\ne k\piup+\dfrac{\piup}2,k\inZ\Bigr\}$； 值域：$ \mathbb{R} $；
          \item 奇偶性：奇函数；
          \item 最小正周期：$ T=\piup  $;
          \item 对称中心：$\left(\dfrac{k\piup}2,0\right)\left(k\inZ\right)$；
